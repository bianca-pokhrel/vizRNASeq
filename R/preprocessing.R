...
#' @param relationship A vector containing variable names for formula to express how the counts for each gene depend on variables. Formatted (group, condition) respectively.
#'
...
mat_conversion <- function(RNA_count_data, sample_info_data, relationship) {
  ...
  DE_convert <- DESeqDataSetFromMatrix(
    ...
    design = ~ relationship[1] + relationship[2]
  )
...
# returns both normalized and un-normalized log2-transformed data
...
#'
#' @return A vector containing both log2 and variance stabilized log2 transformed data as DESeqDataSet objects.
#' @export
#' @import DESeq2
